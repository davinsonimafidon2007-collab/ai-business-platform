...
async def test_estimator_sees_es_fixture_source(monkeypatch: pytest.MonkeyPatch) -> None:
    """Con solo es_market_fixture registrado, estimate usa esa source."""
    ...
    try:
        estimation = await estimator.estimate(vehicle)
    finally:
        ProviderRegistry.clear()

    assert estimation.comparable_count >= 1
    assert any("es_market_fixture" in n for n in estimation.notes)
