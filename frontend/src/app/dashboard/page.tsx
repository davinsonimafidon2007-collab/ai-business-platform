...
export default function DashboardPage() {
  const { data: history, isLoading: historyLoading, isError: historyError, error: historyErrorData } = useSearchHistory();
  const { data: stats, isLoading: statsLoading, isError: statsError, error: statsErrorData } = useDashboardStats();

  // Use real stats from backend if available, otherwise calculate from history
  ...
  const averageResultsPerSearch = stats?.average_results_per_search ?? (totalSearches > 0 ? Math.round(totalVehicles / totalSearches) : 0);
    ...
    <div className="space-y-6">
      ...
      {(historyError || statsError) && (
        <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-red-600 dark:border-red-800 dark:bg-red-900/20 dark:text-red-400">
          Error al cargar datos: {historyErrorData?.message || statsErrorData?.message}
        </div>
      )}

      {/* Stats */}
      {!historyLoading && !statsLoading && !historyError && !statsError && (
        <>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            <StatCard
